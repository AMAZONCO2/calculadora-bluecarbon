...
def load_select_project(id):
    conn = st.connection("postgresql", type="sql")
    projetos_selecionado = conn.query(
        "SELECT * FROM carbono.projetos WHERE id = :id", ttl="3360", params={"id": id}
    )
    resultados_selecionado = conn.query(
        "SELECT * FROM carbono.resultados WHERE projeto_id = :project_id",
        ttl="3360",
            ...
            for _, data in annual_data.iterrows():
                session.execute(
                    query,
                    {
                        ...
                        "ano": int(data["Ano"]),
                        "taxa_desmatamento_projetada": float(
                            data["Taxa de Desmatamento Projetada (%)"]
                        ),
...
def cadastro_projeto():
    ...
    if st.button("Calcular e Salvar Linha de Base e Projeto"):
        ...
        reducoes = resultados_completos(
            int(periodo_projeto),
            estoque_total_inicial,
            fator_risco,
        )
        ...
        project_data = {
            ...
            "Média VCUs": reducoes["VCUs Acumulados (t CO2e)"].sum()
            / len(reducoes["VCUs Acumulados (t CO2e)"]),
...
def relatorio():
    ...
    if not projects.empty:
        ...
        project_options = {
            f"{row['nome_do_projeto']} (ID: {row['id']})": row["id"]
        ...
        if selected_project_keys:
            selected_project_ids = [
                project_options[key] for key in selected_project_keys
            ]
            ...
            for _, project in filtered_projects.iterrows():
                ...
                if not resultados.empty:
                    st.subheader(
                        f"Estimativa de 30 Anos para o Projeto: {project['nome_do_projeto']}"
                    )
                    key_prefix = str(project["id"])

                    with st.expander(f"Editar nome do projeto"):
                        new_name = st.text_input(
                            f"Novo nome do projeto (ID: {project['nome_do_projeto']})",
                            value=project["nome_do_projeto"],
                            key=f"name_{key_prefix}",
                        )
                        if st.button(
                            f"Atualizar nome (ID: {project['id']})",
                            key=f"button_{key_prefix}",
                        ):
                            update_project_name(project["id"], new_name)
                            st.session_state.projects = load_projects()
                            st.success("Nome do projeto atualizado!")

                    media_vcu_anual = resultados["reducao_liquida"].sum() / len(
                        resultados["reducao_liquida"]
                    )
                    ...
                    with st.expander("Mostrar resumo"):
                        ...
                        with col1:
                            ...
                            st.write(
                                f"Período do Projeto: {project['periodo_projeto']:,} anos"
                            )
                            st.write(
                                f"Total de emissão (Vazamento) {anos} anos: {vazamento:,.2f} tCO2e"
                            )
                            st.write(
                                f"Total de emissão (Linha de base) {anos} anos: {linha_base:,.2f} tCO2e"
                            )
                            st.write(
                                f"Total de emissão (Projeto) {anos} anos: {projeto:,.2f} tCO2e"
                            )

                            with col2:
                                st.write(f"Fator de risco: {project['fator_de_risco']:,.2f}")
                                st.write(
                                    f"Taxa de desmatamento: {project['taxa_desmatamento']:,.2f}%"
                                )
                                st.write(f"Buffer: {buffer:,.2f} tCO2e")

                                st.write(f"Total de VCU: {total_vcu:,.2f} tCO2e")
                                st.write(f"Média Anual de VCU: {media_vcu_anual:,.2f} tCO2e")
                    ...
                    chart_vcus_acumulados = (
                        alt.Chart(resultados)
                        .mark_line()
                        .encode(
                            x=alt.X("ano:O", title="Ano"),
                            y=alt.Y(
                                "vcus_acumulados:Q", title="VCUs Acumulados (t CO2e)"
                            ),
                            tooltip=["ano", "vcus_acumulados"],
                        )
                        .properties(title="Progressão dos VCUs Acumulados (t CO2e)")
                        .encode(text="vcus_acumulados:Q")
                    )

                    chart_vcus_acumulados = (
                        chart_vcus_acumulados.mark_line()
                        + chart_vcus_acumulados.mark_text(
                            align="center", baseline="bottom", color="white", fontSize=9
                        ).encode(text=alt.Text("vcus_acumulados:Q", format=".2f"))
                    )
            ...
            chart_area = (
                ...
                .properties(title="Àrea por Projeto")
            )
            ...
            chart_pie = (
                ...
                .encode(
                    theta=alt.Theta(
                        field="VCUs Acumulados Máximo (t CO2e)", type="quantitative"
                    ),
                    color=alt.Color(field="Nome do Projeto", type="nominal"),
                    tooltip=[
                        alt.Tooltip("Nome do Projeto", title="Projeto"),
                        alt.Tooltip(
                            "VCUs Acumulados Máximo (t CO2e)", title="Créditos (tCO2e)"
                        ),
                    ],
                )
                .properties(title="Distribuição dos VCUs Acumulados")
            )
...
st.sidebar.markdown(
    '<p style="font-size=3px">Desenvolvido por ACC © 2024 Amazon Connection Carbon. Todos os direitos reservados.</p>',
