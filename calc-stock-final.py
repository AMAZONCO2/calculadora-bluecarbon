...
def cadastro_projeto():
    ...
    if st.button("Calcular e Salvar Linha de Base e Projeto"):
        ...
        reducoes = resultados_completos(estoque_total_inicial, fator_risco)
...
def relatorio():
    ...
    if not projects.empty:
        selected_projects = st.multiselect(
            ...
            projects['nome_do_projeto'].unique(), placeholder="Selecione uma opção"
        )

        if selected_projects:
            filtered_projects = projects[projects['nome_do_projeto'].isin(selected_projects)]
            ...
            for _, project in filtered_projects.iterrows():
                ...
                if not resultados.empty:   
                    ...
                    media_vcu_anual = resultados['vcus_acumulados'].sum() / len(resultados['vcus_acumulados'])
                    total_vcu = resultados['vcus_acumulados'].max()
                    buffer = resultados['buffer'].max()
                    vazamento = resultados['vazamento'].max()
                    anos = len(resultados['ano']) -1 
                    ...
                    with st.expander("Mostrar resumo"):
                        ...
                        with col1:
                            ...
                            st.write(f"Total de emissão (Linha de base) {anos} anos: {buffer:.2f} tCO2e")
                            st.write(f"Total de emissão (Projeto) {anos} anos: {buffer:.2f} tCO2e")
...
st.sidebar.markdown('<p style="font-size=5px">Desenvolvido por ACC © 2024 Amazon Connection Carbon. Todos os direitos reservados.</p>', unsafe_allow_html=True)
